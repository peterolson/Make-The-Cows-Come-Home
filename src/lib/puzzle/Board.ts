...
export class RectangularBoard extends Board {
    ...
    getValidMovesFrom(coords: RectangularCoords): { destination: RectangularSpace, puller: RectangularSpace }[] {
        return getValidMovesFrom<RectangularCoords, RectangularSpace>(coords, this.getSpaceByCoords.bind(this), RectangularBoard.directions);
    }

    addPiece(coords: RectangularCoords, pieceType: PieceType) {
        const space = this.spaces.find(space => space.coords.x === coords.x && space.coords.y === coords.y);
        if (space) {
...
export class HexagonalBoard extends Board {
    ...
    getSpaceByCoords(coords: HexagonalCoords): HexagonalSpace | undefined {
        return this.spaces.find(space => space.coords.a === coords.a && space.coords.r === coords.r && space.coords.c === coords.c);
    }
    ...
    static directions = [
        ...
        (coords: HexagonalCoords) => ({ a: coords.a, r: coords.r, c: coords.c - 1 }),
        (coords: HexagonalCoords) => ({ a: 1 - coords.a, r: coords.r + coords.a, c: coords.c - (1 - coords.a) }),
        (coords: HexagonalCoords) => ({ a: 1 - coords.a, r: coords.r + coords.a, c: coords.c + coords.a }),
        (coords: HexagonalCoords) => ({ a: 1 - coords.a, r: coords.r - (1 - coords.a), c: coords.c - (1 - coords.a) }),
        (coords: HexagonalCoords) => ({ a: 1 - coords.a, r: coords.r - (1 - coords.a), c: coords.c + coords.a }),
    ];

    getValidMovesFrom(coords: HexagonalCoords): { destination: HexagonalSpace, puller: HexagonalSpace }[] {
        return getValidMovesFrom<HexagonalCoords, HexagonalSpace>(coords, this.getSpaceByCoords.bind(this), HexagonalBoard.directions);
    }
...
function getValidMovesFrom<Coords, Space extends { pieceType: PieceType | null }>(coords: Coords, getSpaceByCoords: (x: Coords) => Space | undefined, directions: ((x: Coords) => Coords)[]): { destination: Space, puller: Space }[] {
    const moves: { destination: Space, puller: Space }[] = [];
    const space = getSpaceByCoords(coords);
    if (!space || (space.pieceType !== PieceType.Person && space.pieceType !== PieceType.Pig)) {
    ...
    for (const direction of directions) {
        ...
        do {
            ...
            if (space.pieceType === PieceType.Person || space.pieceType === PieceType.Pig || (space.pieceType === PieceType.Home && thisType === PieceType.Pig)) {
                if (!prevSpace) {
            ...
            if (space.pieceType === PieceType.Home && thisType === PieceType.Person) {
...
function moveTo<Coords>(from: Coords, to: Coords, getSpaceByCoords: (x: Coords) => Space | undefined) {
...
function serialize(width: number, height: number, hasOddRow: boolean, spaces: Space[]) {
    return `${width}|${height}|${hasOddRow ? 1 : 0}|${spaces.map(space => {
...
function deserialize(object: HexagonalBoard | RectangularBoard, string: string) {
    ...
    const board = object instanceof HexagonalBoard ? new HexagonalBoard(parseInt(width), parseInt(height), !!+hasOddRow) : new RectangularBoard(parseInt(width), parseInt(height));
